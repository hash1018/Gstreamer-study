...
fn send_seek_event(pipeline: &Element, rate: f64) -> bool {
    ...
    if let Ok(video_sink) = pipeline.property("video-sink") {
        if let Ok(video_sink) = video_sink.get::<Element>() {
            video_sink.send_event(seek_event)
        } else {
            println!("failed to update rate2");
            false
        }
    } else {
        println!("failed to update rate");
        false
    }

    /*
    // If we have not done so, obtain the sink through which we will send the seek events
    if let Ok(Some(video_sink)) = pipeline.property::<Element>("video-sink") {
 ...
 'Q' to quit"
    ...
    ready_rx.attach(Some(&main_loop.context()), move |command: Command| {
        ...
        match command {
            ...
            Command::NextFrame => {
                /*
                if let Ok(Some(video_sink)) = pipeline.property::<Option<Element>>("video-sink")
                {
                    // Send the event
...
fn main() {
    match common::run(tutorial_main) {
        Ok(_) => {}
